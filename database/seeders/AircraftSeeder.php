...
class AircraftSeeder extends Seeder
{
    public function run()
    {
        $aircraftData = [
            ...
            ['name' => 'Spitfire Mk IX', 'Description' => 'British WW2 fighter aircraft', 'body' =>"The Supermarine Spitfire is a British single-seat fighter aircraft used by the Royal Air Force and other Allied countries before, during, and after World War II. It was the only British fighter produced continuously throughout the war. The Spitfire remains popular among enthusiasts. Around 70 remain airworthy, and many more are static exhibits in aviation museums throughout the world."]
        ];

        $museumSmithsonian = Location::where('name', 'Smithsonian National Air and Space Museum')->first();
        $museumRAF = Location::where('name', 'Royal Air Force Museum London')->first();
        $museumFlight = Location::where('name', 'The Museum of Flight')->first();
        ...
        foreach ($aircraftData as $data) {
            ...
            switch ($data['name']) {
                ...
                case 'F-22 Raptor':
                    ...
                    $aircraft->locations()->attach([$museumFlight->id]);
                    break;
                case 'Cessna 172':
                ...
                case 'Spitfire Mk IX':
                    $tags= Tag::whereIn('name', ['Military'])->get();
                    $aircraft->locations()->attach([$museumSmithsonian->id, $museumRAF->id]);
                    break;
